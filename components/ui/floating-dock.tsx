...
const FloatingDockDesktop = ({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "mx-auto hidden md:flex h-16 gap-4 items-end rounded-2xl bg-zinc-900 px-4 pb-3",
        className
      )}
    >
      {items.map((item) => (
  ...
  return (
    <Link href={href}>
      <motion.div
        ...
        className="aspect-square rounded-full bg-zinc-600 flex items-center justify-center relative"
      >
        <AnimatePresence>
          {hovered && (
            <motion.div
        ...
        <motion.div
          ...
          className="flex items-center justify-center svg-stroke-white"
        >
          {icon}
        </motion.div>
      </motion.div>
